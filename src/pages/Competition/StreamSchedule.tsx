            ...
            {scheduleDays.map(({ date, dateParts }) => (
              <Fragment key={date}>
                ...
                {getActivitiesByDate(date)
                  .sort(byDate)
                  .map((activity) => {
                    const { eventId, roundNumber, groupNumber } =
                    ...
                    const roomName =
                      activity?.room?.name || activity?.parent?.room?.name;
                    const roomColor =
                      activity?.room?.color || activity?.parent?.room?.color;
                    ...
                    return (
                      <Link
                        ...
                        <td className="py-2 text-center">
                          <span
                            className={`cubing-icon event-${eventId} mx-1 text-xl`}
                          />
                        </td>
